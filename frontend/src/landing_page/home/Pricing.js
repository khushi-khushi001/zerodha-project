import React from 'react';

function Pricing() {
    return (
        <div className='container p-5'>
            <div className='row'>
                <div className='col-4'>
                    <h1 className='mb-3 fs-2'>Unbeatable Price</h1>
                    <p style={{fontSize: "15px"}}>We pioneered the concept of discount broking and price transperency in India.
                        Flat fees and no hidden charges.
                    </p>
                    <a href='/' style={{textDecoration:"none"}}>See Pricing <i class="fa-solid fa-arrow-right-long"></i></a>
                </div>
                <div className='col-2'></div>
                <div className='col-4'>
                    <div className='row text-center'>
                        <div className='col p-3 border'>
                            <h1 className='fs-4 mb-3'><i class="fa-solid fa-indian-rupee-sign"></i>0</h1>
                            <p style={{fontSize: "12px"}}>Free equity delivery and  direct mutual funds</p>
                        </div>
                        <div className='col p-3 border '>
                            <h1 className='fs-4 mb-3'><i class="fa-solid fa-indian-rupee-sign"></i>20</h1>
                            <p style={{fontSize: "12px"}}>Intraday and F&O</p>
                        </div>
                    </div>
                    
                </div>
            </div>
        </div>
      );
}

export default Pricing;